...
from src.net import ElasticNet, AdaptiveElasticNet
...
class VAR:
    ...
    def fit(self, X, y=None, exog=None, method='OLS', **kwargs):
        ...
        if method == 'OLS':
            ...
            raise Error('Fit method not implemented.')
    ...
    def _build_X_block(self, var_data, add_intercepts=True, exog_data=None):
        ...
        if exog_data is not None:
            assert var_data.shape[0] == exog_data.shape[0],\
    ...
    def _build_X(self, var_data, add_intercepts=True, exog_data=None):
        ...
        # build
        X_block = self._build_X_block(var_data, add_intercepts=add_intercepts, exog_data=exog_data)
    ...
    def fit_ols(self, X, y=None, exog=None, return_model=False):
        ...
        # build inputs
        var_data = X.copy()
        X, y = self._build_X_y(var_data, add_intercepts=self.add_intercepts, exog_data=exog)
    ...
    def _split_coef(self, var_data, exog_data=None):
        ...
        # split & store
        self.intercepts_ = self._extract_intercepts(m_features=m_features, n_series=n_series, a_const=a_const)
        self.exog_loadings_ = self._extract_exog_loadings(m_features=m_features, k_exog=k_exog, n_series=n_series, a_const=a_const)
        self.var_matrices_ = self._extract_var_matrices(m_features=m_features, n_series=n_series, k_exog=k_exog, a_const=a_const)
        
    def _extract_intercepts(self, m_features, n_series, a_const):
        '''Returns a numpy array of intercepts.'''
        indices = list(np.concatenate([np.arange(0, 0+a_const)+m_features*i for i in range(n_series)]))
    ...
    def _extract_exog_loadings(self, a_const, m_features, k_exog, n_series):
        ...
        indices = list(np.concatenate([np.arange(a_const, a_const+k_exog)+m_features*i for i in range(n_series)]))
    ...
    def _extract_var_matrices(self, m_features, n_series, k_exog, a_const):
        ...
        for l in range(self.p_lags):
            indices = list(np.concatenate([np.arange(a_const+k_exog, a_const+k_exog+n_series)+m_features*i+n_series*l for i in range(n_series)]))
    ...
    def predict(self, var_data, exog_data=None):
        ...
        t_periods = var_data.shape[0] - self.p_lags
        n_series = var_data.shape[1]
        X  = self._build_X(var_data, add_intercepts=self.add_intercepts, exog_data=exog_data)
        
        # make predictions
        y_pred = (X @ self.coef_).reshape(t_periods, n_series, order='F')
        y_pred = pd.DataFrame(y_pred,
                              index=var_data.index[self.p_lags:],
    ...
    def df_used_(self):
        ...
        '''
        df_used = np.count_nonzero(self.intercepts_)\
    ...
    def _scale_coefs(self, model, var_data, exog_data=None):
        ...
        x_block = self._build_X_block(var_data, add_intercepts=False, exog_data=exog_data)
    ...
    def fit_elastic_net(self, X, y=None,
                        exog=None,
                        alpha=0.1,
                        lambdau=0.1,
        ...
        X, y = self._build_X_y(scaled_var_data, add_intercepts=False, exog_data=scaled_exog)
    ...
    def fit_adaptive_elastic_net(self, X, y=None,
                        exog=None,
                        alpha=0.1,
                        lambdau=0.1,
                        penalty_weights=None,
        ...
        # estimate
        model = AdaptiveElasticNet(alpha=alpha,
                                   lambdau=lambdau,
                                   intercept=False,
                                   standardize=False,
                                   **kwargs)
    ...
    def fit_elastic_net_cv(self, X, y=None,
                           exog=None,
                           grid=None,
                           folds=12,
                           return_cv=False,
        ...
        # estimate
        cv = GridSearchCV(elnet, grid, cv=split, n_jobs=-1, verbose=1, return_train_score=True, **kwargs)
        cv.fit(X, y, penalty_weights=None)
        
        # store estimates
        self.coef_ = self._scale_coefs(model=cv.best_estimator_, var_data=var_data, exog_data=exog)
        self._split_coef(var_data, exog_data=exog)
    ...
    def fit_adaptive_elastic_net_cv(self, X, y=None,
                           exog=None,
                           grid=None,
                           folds=12,
                           return_cv=False,
        ...
        elnet.fit(X, y, ini_split=split) #required to update the penalty weights only once
...
class OldVAR:
    ...
    def _penalty_weights(self, method='OLS', penalise_diagonal=True, **kwargs):
        ...
        if method == 'OLS':
            ...
            coefficients = self.fit_elastic_net(return_model=True, **kwargs).coef_ + 1e-8
        elif method == 'ridge':
        ...
        zero_penalty_indices = [self.m_features*i for i in range(self.n_series)] #intercepts
        if not penalise_diagonal:
            zero_penalty_indices += [1+(self.m_features+1)*i for i in range(self.n_series)] #VAR1 diagonal
    ...
    def fit_elastic_net_cv(self, grid, folds=12, return_cv=False, weighting_method='OLS', penalise_diagonal=True, **kwargs):
        '''Fits the FAVAR coefficients using the glmnet routine.
        ...
        if weighting_method == 'equal':
            penalty_weights = self._penalty_weights(method='equal', penalise_diagonal=penalise_diagonal)
        elif weighting_method == 'OLS':
            penalty_weights = self._penalty_weights(method='OLS', penalise_diagonal=penalise_diagonal)
...
#             lambdau = ElasticNet(intercept=False, standardize=False).find_lambda(X, y, lambda_grid=grid['lambdau'], alpha=0.5)
            lambdau = ElasticNet(intercept=False, standardize=False).find_lambda(X, y, lambda_grid=np.unique(np.concatenate([np.geomspace(1e-5, 1e5, 25), np.linspace(1e-5, 1e5, 25)])), alpha=0.01)
            penalty_weights = self._penalty_weights(method='ElasticNet', penalise_diagonal=penalise_diagonal, alpha=0.01, lambdau=lambdau)
...
#             lambdau = ElasticNet(intercept=False, standardize=False).find_lambda(X, y, lambda_grid=grid['lambdau'], alpha=0)
            lambdau = ElasticNet(intercept=False, standardize=False).find_lambda(X, y, lambda_grid=np.unique(np.concatenate([np.geomspace(1e-5, 1e5, 25), np.linspace(1e-5, 1e5, 25)])), alpha=0)
            penalty_weights = self._penalty_weights(method='ridge', penalise_diagonal=penalise_diagonal, alpha=0, lambdau=lambdau)
        else:
        ...
        self.mean_shrinkage_ = 1-(abs(self.var_1_matrix_[self.var_1_matrix_!=0])/abs(ols_estimate[self.var_1_matrix_!=0])).mean()
        if return_cv:
            return cv
