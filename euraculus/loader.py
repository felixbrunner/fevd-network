...
def load_monthly_crsp(year, month, which='back', column=None):
    ...
    # load raw
    if which == 'back':
        df = pd.read_csv('../data/processed/monthly/{}/{}/df_back.csv'.format(year, month))
    elif which == 'forward':
        df = pd.read_csv('../data/processed/monthly/{}/{}/df_forward.csv'.format(year, month))
...
def load_monthly_estimation_data(year, month, column=None):
    ...
    '''
    # load raw
    df_back = pd.read_csv('../data/processed/monthly/{}/{}/df_back.csv'.format(year, month))
    ...
    # merge
    df = df_back.merge(df_back_residuals, on=['date', 'permno'], how='outer')\
                .merge(df_var_decomposition, on=['date', 'permno'], how='outer')
