...
# %%
corr_heatmap(
    prec_to_pcorr(glasso_cv.best_estimator_.precision_),
...
# %%
_ = data.lookup_ticker(tickers=["FIS", "BKNG"], date=sampling_date)
_
...
mean_size = df_info.mean_size
ax.bar(height=index_decomp/index_decomp.sum(), x=range(100), label="index variance contribution share")
ax.bar(height=mean_size/mean_size.sum(), x=range(100), color="r", alpha=0.5, label="index weight")
...
# %%
fig, ax = plt.subplots(1, 1)
ax.plot((index_decomp/index_decomp.sum()).cumsum(), label="index variance contribution share")
ax.plot((mean_size/mean_size.sum()).cumsum().reset_index(drop=True), label="index weight")
ax.legend()
