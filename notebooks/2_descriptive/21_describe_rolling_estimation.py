...
add_recession_bars(
    ax1, freq="M", startdate=df_stats.index[0], enddate=df_stats.index[-1]
)
...
if save_outputs:
    save_ax_as_pdf(ax1, save_path="../reports/rolling/estimation_summary.pdf")
...
if save_outputs:
    save_ax_as_pdf(ax1, save_path="../reports/rolling/network_summary.pdf")
...
# %%
df_estimates["var_factor_loadings_crsp"].unstack().mean(axis=1).plot(figsize=(20, 4))
...
# %%
df_estimates["fev_fullout_connectedness"] = (
    df_estimates["fev_out_connectedness"] + df_estimates["fev_self_connectedness"]
)
df_estimates["fev_fullout_connectedness_weighted"] = (
    df_estimates["wfev_out_connectedness"] + df_estimates["wfev_self_connectedness"]
)
df_estimates["fevd_fullout_connectedness"] = (
    df_estimates["fevd_out_connectedness"] + df_estimates["fevd_self_connectedness"]
)
df_estimates["fevd_fullout_connectedness_weighted"] = (
    df_estimates["wfevd_out_connectedness"] + df_estimates["wfevd_self_connectedness"]
)
df_estimates["fev_net_connectedness"] = (
    df_estimates["fev_out_connectedness"] - df_estimates["fev_in_connectedness"]
)
df_estimates["fevd_net_connectedness"] = (
    df_estimates["fevd_out_connectedness"] - df_estimates["fevd_in_connectedness"]
)

# %%
df_estimates.iloc[-100:, :][
    ["fev_out_page_rank_85", "ticker", "mean_mcap_rank", "ff_sector_ticker"]
].sort_values("fev_out_page_rank_85", ascending=False).iloc[:10]

# %%
df_estimates[
    [
        "fev_out_page_rank_85",
        "ticker",
        "comnam",
        "sic_division",
        "ff_sector",
...
].groupby("sampling_date").apply(
    lambda x: x.sort_values("fev_out_page_rank_85", ascending=False)
)

# %%
# influence_variable = "fevd_fullout_connectedness"
influence_variable = "wfevd_out_connectedness"
...
]  # .reset_index().set_index(["sampling_date", "ticker"])
_df[["sic_division", "ff_sector", "ff_sector_ticker", "gics_sector"]] = _df[
    ["sic_division", "ff_sector", "ff_sector_ticker", "gics_sector"]
].fillna("N/A")
...
def construct_weights(
    df_estimates: pd.DataFrame, weighting_variable: str, grouping_variables: list = None
):
    """
    ...
    # df_weights = _df.values.reshape(-1, 1)

    return df_weights


# %%
mcap_weights = construct_weights(
    df_estimates=df_estimates,
    weighting_variable="mean_mcap",
    grouping_variables=["sampling_date", "permno"],
)
network_weights = construct_weights(
    df_estimates=df_estimates,
    weighting_variable="wfevd_full_out_connectedness",
    grouping_variables=["sampling_date", "permno"],
)
...
def plot_leverage(df_estimates: pd.DataFrame, grouping_variables: list = None):
    ...
    for permno, series in df_leverage.iteritems():
        ...
        if type(ticker) != str:  # np.isnan(ticker):
        ...
        if max_leverage > 2:
            ax.plot(
                series,
                color=sector_colors[ff_sector_codes[ff_sector_ticker]],
                linewidth=1.5,
                alpha=1,
            )
            ax.text(
                series.index[max_leverage_pos],
                max_leverage + 0.05,
                s=ticker,
    ...
    add_recession_bars(
        ax, startdate=df_leverage.index[0], enddate=df_leverage.index[-1]
    )
    # legends
    ...
    ax.set_title(
        "Leverage effect of FEVD spillover network on non-systematic innovations in the wealth portfolio"
    )
    ax.set_ylabel("Network multiplier")
    ax.set_yscale("log")
    print(count)


# %%
_ = plot_leverage(df_estimates)
...
def make_indices(df_estimates, df_historic, df_future):
    ...
    df_historic = df_historic.join(
        ...
        network_weights.stack().rename("network_weights"),
        on=["sampling_date", "permno"],
    )
    ...
    df_indices["hist_vw"] = (
        (df_historic["mcap_weights"] * df_historic["retadj"]).unstack().sum(axis=1)
    )
    df_indices["hist_nw"] = (
        (df_historic["network_weights"] * df_historic["retadj"]).unstack().sum(axis=1)
    )
    df_indices["hist_nw_resid"] = (
        (
            (df_historic["network_weights"] - df_historic["mcap_weights"])
            * df_historic["retadj"]
        )
        .unstack()
        .sum(axis=1)
    )
    df_indices["hist_lw_resid"] = (
        (
            (df_historic["network_weights"] / df_historic["mcap_weights"])
            / (df_historic["network_weights"] / df_historic["mcap_weights"]).sum()
            * df_historic["capm_resid"]
        )
        .unstack()
        .sum(axis=1)
    )
    ...
    df_indices["fut_vw"] = (
        (df_future["mcap_weights"] * df_future["retadj"]).unstack().sum(axis=1)
    )
    df_indices["fut_nw"] = (
        (df_future["network_weights"] * df_future["retadj"]).unstack().sum(axis=1)
    )
    df_indices["fut_nw_resid"] = (
        (
            (df_future["network_weights"] - df_future["mcap_weights"])
            * df_future["retadj"]
        )
        .unstack()
        .sum(axis=1)
    )
    df_indices["fut_lw_resid"] = (
        (
            (df_future["network_weights"] / df_future["mcap_weights"])
            / (df_future["network_weights"] / df_future["mcap_weights"]).sum()
            * df_future["capm_resid"]
        )
        .unstack()
        .sum(axis=1)
    )
...
# %%
df_indices.resample("M").apply(lambda x: (x + 1).prod() - 1).join(
    df_spy.unstack()[[("ret_excess", "vw"), ("ret_excess_next12M", "vw")]]
).corr()
...
ax.plot(
    index.rolling(252).apply(lambda x: (x + 1).prod() - 1),
    label="Rolling annualized return",
)
ax.plot(
    index.rolling(252).apply(lambda x: x.std() * np.sqrt(252)),
...
index = (
    (
        (_["network_weights"] / _["mcap_weights"])
        / (_["network_weights"] / _["mcap_weights"]).sum()
        * _["capm_resid"]
    )
    .unstack()
    .sum(axis=1)
...
# %%
mcap_weights = (
    df_estimates.groupby(["sampling_date", "ff_sector_ticker"])["mean_mcap"]
    .sum()
    .unstack()
)
...
network_weights = (
    df_estimates.groupby(["sampling_date", "ff_sector_ticker"])[
        "fevd_fullout_connectedness_weighted"
    ]
    .sum()
    .unstack()
)
...
industry_totals = (
    _df.groupby(["sampling_date", "ff_sector_ticker"])[influence_variable]
    .sum()
    .unstack()
)
...
ax.set_title(
    f"Page rank decomposition of valuation uncertainty by sector ({influence_variable})"
)
ax.set_xlabel("Date")
...
add_recession_bars(
    ax, freq="M", startdate=industry_totals.index[0], enddate=industry_totals.index[-1]
)
...
_ = (
    bump(df_estimates, column=column, n_tickers=100)["rank"]
    .unstack()
    .rolling(12)
    .mean()
)
...
# %%
bump(df_estimates, column="fev_out_eigenvector_centrality", n_tickers=100)[
    "rank"
].unstack().rolling(
    12, min_periods=6
).mean()  # .plot()
...
# %%
date = "2000-12-31"
df_estimates.reset_index().set_index(["sampling_date", "ticker"])[
    "fev_out_connectedness"
][date].sort_values().plot.bar(
    title=date
)  # .pie(title=date)
...
# %%
date = "2021-12-31"
df_estimates.reset_index().set_index(["sampling_date", "ticker"])["fev_out_page_rank"][
    date
].sort_values().iloc[-10:].plot.pie(title=date, normalize=True)
...
# %%
df_estimates["fev_out_page_rank"].groupby("sampling_date").apply(
...
(
    df_stats["fevd_avg_connectedness"]
    * df_stats["fevd_concentration_out_connectedness"]
).plot()
