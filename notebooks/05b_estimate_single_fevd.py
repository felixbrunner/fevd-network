...
# %%
data = DataMap("../data")
df_idio_var = data.load_sample(year=sampling_date.year, month=sampling_date.month, column=["var_idio"])
df_var = data.load_sample(year=sampling_date.year, month=sampling_date.month, column=["var"])
...
# %%
ticker_list = data.load_sample(year=sampling_date.year, month=sampling_date.month, column=["ticker"]).tail(1).values.ravel().tolist()
...
# %%
# get data
df_lookup = data.lookup_ticker(tickers=ticker_list, date=sampling_date)

# create output
df_tickers =  pd.Series(dtype='object', name='Company Name', index=pd.Index(data=[], name='Ticker'))

# fill in values from lookup
for ticker, unique_ticker in zip(ticker_list, column_to_ticker.values()):
    df_tickers[unique_ticker] = df_lookup[df_lookup.ticker == ticker].comnam.values[0].replace('&', '\&').title()

# store
df_tickers = df_tickers.to_frame().sort_index()
# df_tickers.export_to_latex(filename='tickers.tex', path='../reports/tables/')
data.store(data=df_tickers, path="samples/{0}{1:0=2d}/df_tickers.csv".format(sampling_date.year, sampling_date.month))
...
# %%
euraculus.plot.corr_heatmap(euraculus.utils.autocorrcoef(df_var, lag=1), title='Total Variance Auto-Correlations (First order)',
                         save_path='../reports/figures/estimation/heatmap_total_variance_autocorrelation.pdf'
                           )

# %%
euraculus.plot.corr_heatmap(df_var.corr(), title='Total Variance Correlations',
#                          save_path='../reports/figures/estimation/heatmap_total_variance_correlation.pdf',
                           )

# %%
euraculus.plot.missing_data(df_idio_var,
...
# %%
euraculus.plot.histogram(df_idio_var.fillna(0).stack(), title='Distribution of Raw Data', drop_tails=0.01, bins=100,
#                           save_path='../reports/figures/estimation/histogram_raw_data.pdf',
                        )

# %%
euraculus.plot.var_timeseries(df_idio_var, total_var=df_var, index_var=df_spy_var,
...
# %%
euraculus.plot.corr_heatmap(df_log_idio_var.corr(), title='Idiosyncratic Variance Correlations',
#                       save_path='../reports/figures/estimation/heatmap_idiosyncratic_variance_correlation.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(euraculus.utils.autocorrcoef(df_log_idio_var, lag=1), title='Idiosyncratic Variance Auto-Correlations (First order)',
#                          save_path='../reports/figures/estimation/heatmap_idiosyncratic_variance_autocorrelation.pdf',
                           )

# %%
pd.Series(np.diag(euraculus.utils.autocorrcoef(df_log_idio_var, lag=1))).plot(kind='hist', title='Diagonal Autocorrelations', bins=10)
plt.show()

# %%
euraculus.plot.histogram(df_log_idio_var.stack(),  title='Distribution of Idiosyncratic Variances',
...
# %%
euraculus.plot.corr_heatmap(var.var_1_matrix_, title='Non-regularized VAR(1) coefficients (OLS)', infer_limits=True,
...
# %%
euraculus.plot.net_cv_contour(var_cv, 15, logx=True, logy=True,
#                             save_path='../reports/figures/estimation/contour_var.pdf',
                       )

# %%
euraculus.plot.net_scatter_losses(var_cv,
...
# %%
euraculus.plot.corr_heatmap(var.var_1_matrix_, title='Regularized VAR(1) coefficients (Adaptive Elastic Net)', infer_limits=True,
...
euraculus.plot.network_graph(vargraph, column_to_ticker, linewidth=0.02, red_percent=2, title='VAR network links',
...
# %%
euraculus.plot.corr_heatmap(residuals.corr(), title='VAR Residual Correlation',
#                           save_path='../reports/figures/estimation/heatmap_VAR_residual_correlation.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(euraculus.utils.autocorrcoef(residuals, lag=1), title='VAR Residual Auto-Correlation (First order)',
...
# %%
euraculus.plot.histogram(residuals.stack(),  title='Distribution of VAR Residuals',
#                        save_path='../reports/figures/estimation/histogram_VAR_residuals.pdf',
                  )

# %%
euraculus.plot.corr_heatmap(residuals.cov(), title='Sample Estimate of the VAR Residual Covariance Matrix', infer_limits=True,
#                           save_path='../reports/figures/estimation/heatmap_VAR_residual_covariance.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(euraculus.utils.prec_to_pcorr(np.linalg.inv(residuals.cov())),
...
# %%
euraculus.plot.cov_cv_contour(cov_cv, 15, logx=False, logy=False,
#                            save_path='../reports/figures/estimation/contour_cov.pdf',
                             )

# %%
euraculus.plot.cov_scatter_losses(cov_cv,
...
euraculus.plot.corr_heatmap(covar_ate, title='Adaptive Threshold Estimate of VAR Residual Covariance Matrix',
                          vmin=-lim, vmax=lim,
#                           save_path='../reports/figures/estimation/heatmap_cov_matrix_ate.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(residuals.cov().values - covar_ate, 'Shrinkage difference', infer_limits=True)

# %%
euraculus.plot.corr_heatmap(euraculus.utils.prec_to_pcorr(np.linalg.inv(covar_ate)), 
...
# %%
euraculus.plot.plot_glasso_cv(glasso_cv,
#                         save_path='../reports/figures/estimation/line_cov_cv.pdf',
                       )

# %%
lim = residuals.cov().abs().values.max()
euraculus.plot.corr_heatmap(covar, title='Graphical Lasso Estimate of VAR Residual Covariance Matrix',
                          vmin=-lim, vmax=lim,
#                           save_path='../reports/figures/estimation/heatmap_cov_matrix.pdf',
                     )

# %%
euraculus.plot.corr_heatmap(residuals.cov().values - covar, 'Shrinkage difference', infer_limits=True)

# %%
euraculus.plot.corr_heatmap(glasso_cv.best_estimator_.precision_, infer_limits=True,
                      title='Graphical Lasso Estimate of VAR Residual Precision Matrix')

# %%
euraculus.plot.corr_heatmap(euraculus.utils.prec_to_pcorr(glasso_cv.best_estimator_.precision_),
...
euraculus.plot.network_graph(covgraph, column_to_ticker, linewidth=0.5, red_percent=0, title='Covariance Matrix as Undirected Graph')
...
euraculus.plot.network_graph(congraph, column_to_ticker, linewidth=1, red_percent=0, title='Contemporaneous Network Links',
...
# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(0), title='VMA(0) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
#                       save_path='../reports/figures/estimation/heatmap_vma0_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(1), title='VMA(1) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
#                       save_path='../reports/figures/estimation/heatmap_vma1_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(2), title='VMA(2) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
#                       save_path='../reports/figures/estimation/heatmap_vma2_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(3), title='VMA(3) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
#                       save_path='../reports/figures/estimation/heatmap_vma3_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(5), title='VMA(5) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
#                       save_path='../reports/figures/estimation/heatmap_vma5_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(10), title='VMA(10) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
#                       save_path='../reports/figures/estimation/heatmap_vma10_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.vma_matrix(21), title='VMA(21) Matrix', infer_limits=True, #, vmin=-lim, vmax=lim,
...
# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(0), title='Impulse Response(0) Matrix', infer_limits=True,
#                       save_path='../reports/figures/estimation/heatmap_ir0_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(1), title='Impulse Response(1) Matrix', infer_limits=True,
#                       save_path='../reports/figures/estimation/heatmap_ir1_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(2), title='Impulse Response(2) Matrix', infer_limits=True,
#                       save_path='../reports/figures/estimation/heatmap_ir2_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(3), title='Impulse Response(3) Matrix', infer_limits=True,
#                       save_path='../reports/figures/estimation/heatmap_ir3_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(5), title='Impulse Response(5) Matrix', infer_limits=True,
#                       save_path='../reports/figures/estimation/heatmap_ir5_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(10), title='Impulse Response(10) Matrix', infer_limits=True,
#                       save_path='../reports/figures/estimation/heatmap_ir10_matrix.pdf',
                           )

# %%
euraculus.plot.corr_heatmap(fevd.impulse_response(21), title='Impulse Response(21) Matrix', infer_limits=True,
...
# %%
euraculus.plot.corr_heatmap(pd.DataFrame(fevd.fev_single(horizon))-np.diag(np.diag(fevd.fev_single(horizon))),
...
# %%
euraculus.plot.corr_heatmap(pd.DataFrame(fevd.decompose_fev(horizon=horizon, normalise=False))-np.diag(np.diag(fevd.decompose_fev(horizon=horizon, normalise=False))),
                      title='FEV Decomposition', vmin=0, vmax=None, cmap='binary',
#                       save_path='../reports/figures/estimation/heatmap_FEV_decomposition.pdf',
                     )

# %%
euraculus.plot.corr_heatmap(pd.DataFrame(fevd.decompose_fev(horizon=horizon, normalise=True))-np.diag(np.diag(fevd.decompose_fev(horizon=horizon, normalise=True))),
...
# %%
euraculus.plot.corr_heatmap(pd.DataFrame(fevd.fu_single(horizon))-np.diag(np.diag(fevd.fu_single(horizon))),
...
# %%
euraculus.plot.corr_heatmap(pd.DataFrame(fevd.decompose_fu(horizon=horizon, normalise=False))-np.diag(np.diag(fevd.decompose_fu(horizon=horizon, normalise=False))),
                      title='FU Decomposition', vmin=0, vmax=None, cmap='binary',
#                       save_path='../reports/figures/estimation/heatmap_FU_decomposition.pdf',
                     )

# %%
euraculus.plot.corr_heatmap(pd.DataFrame(fevd.decompose_fu(horizon=horizon, normalise=True))-np.diag(np.diag(fevd.decompose_fu(horizon=horizon, normalise=True))),
...
# %%
euraculus.plot.network_graph(fevd.to_fev_graph(horizon, normalise=False), column_to_ticker, title='FEVD Network (FEV absolute)', red_percent=5, linewidth=0.25,
#                        save_path='../reports/figures/estimation/network_FEV_absolute.png',
                      )

# %%
euraculus.plot.network_graph(fevd.to_fev_graph(horizon, normalise=True), column_to_ticker, title='FEVD Network (FEV %)', red_percent=2, linewidth=0.25,
#                        save_path='../reports/figures/estimation/network_FEV_normalised.png',
                      )

# %%
data.lookup_ticker(tickers="ABBV", date=sampling_date)

# %% [markdown]
# #### FU Adjacency

# %%
euraculus.plot.network_graph(fevd.to_fu_graph(horizon, normalise=False), column_to_ticker, title='FEVD Network (FU absolute)', red_percent=5, linewidth=0.25,
#                        save_path='../reports/figures/estimation/network_FU_absolute.png',
                      )

# %%
euraculus.plot.network_graph(fevd.to_fu_graph(horizon, normalise=True), column_to_ticker, title='FEVD Network (FU %)', red_percent=2, linewidth=0.25,
#                        save_path='../reports/figures/estimation/network_FU_normalised.png',
                      )

# %%
data.lookup_ticker(tickers=["TFC", "XON"], date=sampling_date)
